...
class DataTrainingArguments:
    ...
    source_prefix: Optional[str] = field(
        default="Instruction: Compress the information in the retrieved documents into a 2-sentence summary that could be used to answer the question.", metadata={"help": "A prefix to add before every source text (useful for T5 models)."}
    )
...
def main():
    ...
    # overwrite
    # training_args.output_dir = '{}-{}-{}'.format(training_args.output_dir, model_args.model_name_or_path.replace("/", "-"),
    #                                     datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    print("Output dir: {}".format(training_args.output_dir))
    ...
    def preprocess_summary_function(split, examples):
        ...
        for i in range(len(examples['question'])):
            Document = ""
            for j in range(len(examples['ctxs'][i])):
                Document += examples['ctxs'][i][j]['text'] + "\n"
            input_txt = "Question: {}\n Document: {}\n Summary: ".format(
                examples['question'][i],
                Document,
            )
            inputs.append(input_txt)
            target = examples['summary'][i]
    ...
    if training_args.do_train:
        ...
        with training_args.main_process_first(desc="train dataset map pre-processing"):
            train_dataset = train_dataset.map(
                partial(preprocess_summary_function, 'train'),
                batched=True,
    ...
    if training_args.do_eval:
        ...
        with training_args.main_process_first(desc="validation dataset map pre-processing"):
            eval_dataset = eval_dataset.map(
                partial(preprocess_summary_function, 'dev'),
                batched=True,
                num_proc=data_args.preprocessing_num_workers,
                remove_columns=column_names,
                load_from_cache_file=not data_args.overwrite_cache,
    ...
    def compute_metrics(eval_preds):
        ...
        # postprocess_text 호출 전에 입력값 확인
        if not decoded_preds or not decoded_labels:
            raise ValueError("Predictions or labels are empty!")
    ...
    trainer = Seq2SeqTrainer(
        ...
        # callbacks=[DisableWandbCallback]
    )
